...
class Match:
    ...
    def run(self, team1, team2, num_player=1, max_frames=MAX_FRAMES, max_score=3, record_fn=None,
            ...
            initial_ball_velocity=[0, 0], save_images=None, data_callback=None, verbose=False):
        ...
        for it in range(max_frames):
            ...
            if t1_type == 'image':
                team1_actions_delayed = self._r(team1.act)(team1_state, team1_images, soccer_state) # Need to remove soccer_state after data collection
            else:
                team1_actions_delayed = self._r(team1.act)(team1_state, team2_state, soccer_state)

            if t2_type == 'image':
                team2_actions_delayed = self._r(team2.act)(team2_state, team2_images, soccer_state) # Need to remove soccer_state after data collection
            else:
            ...
            # Use verbose for viewing match live (like hw5)
            if verbose:
                proj = np.array(team1_state[0]['camera']['projection']).T
                view = np.array(team1_state[0]['camera']['view']).T
                ax.clear()
                ax.imshow(team1_images[0])
                WH2 = np.array([self.screen_width, self.screen_height]) / 2
                ax.add_artist(plt.Circle(WH2*(1+self._to_image(team1_state[0]['kart']['location'], proj, view)), 2,
